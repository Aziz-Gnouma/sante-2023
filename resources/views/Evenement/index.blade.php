      ...
      <div class="container mx-auto px-6 py-2">
          <div class="text-right">
            @can('Evenement create')
              <a href="{{route('admin.evenements.create')}}" class="bg-blue-500 text-white font-bold px-5 py-1 rounded focus:outline-none shadow hover:bg-blue-500 transition-colors ">New post</a>
            @endcan
          </div>
            ...
            <tbody>
              @can('Evenement access')
                ...
                <tr class="hover:bg-grey-lighter">
                  <td class="py-4 px-6 border-b border-grey-light">{{ $Evenement->title }}</td>
                  ...
                  <td class="py-4 px-6 border-b border-grey-light text-right">
                    ...
                    @can('Evenement edit')
                    <a href="{{route('admin.evenements.edit',$Evenement->id)}}" class="text-grey-lighter font-bold py-1 px-3 rounded text-xs bg-green hover:bg-green-dark text-blue-400">Edit</a>
                    @endcan

                    @can('Evenement delete')
                    <form action="{{ route('admin.evenements.destroy', $Evenement->id) }}" method="POST" class="inline">
                        @csrf
                        @method('delete')
